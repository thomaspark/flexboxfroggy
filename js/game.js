var game = {
  ...
  setupAutocomplete: function () {
    ...
    $code.on(
      ...
      this.debounce(function () {
        ...
        var inputUpToCursor = $code.val().substring(0, cursorPosition);
        var lastLine = inputUpToCursor.split(";").pop().trim();

        // Only show suggestions if we're not inside a property value
        if (!lastLine.includes(":")) {
          var lastWord = lastLine.split(/\s+/).pop();
          if (lastWord) {
            var suggestions = game.autocompleteCSSProperties(lastWord);
            game.showAutocompleteSuggestions(suggestions);
          } else {
            game.hideAutocompleteSuggestions();
          }
        } else {
          game.hideAutocompleteSuggestions();
        }
    ...
    $code.on("keydown", function (e) {
      if (e.key === "Tab" && $("#autocomplete-suggestions").is(":visible")) {
        e.preventDefault();
        game.selectAutocompleteSuggestion();
      }
    });

    $(document).on("click", function (e) {
      if (!$(e.target).closest("#autocomplete-suggestions, #code").length) {
        game.hideAutocompleteSuggestions();
      }
    });
  },

  autocompleteCSSProperties: function (input) {
    const properties = [
      "justify-content",
      "align-items",
      "flex-direction",
  ...
  showAutocompleteSuggestions: function (suggestions) {
    ...
    suggestions.forEach(function (suggestion) {
      $("<div>")
        ...
        .text(suggestion)
        .on("click", function () {
          game.insertAutocompleteSuggestion(suggestion);
        })
  ...
  hideAutocompleteSuggestions: function () {
    $("#autocomplete-suggestions").hide();
  },

  selectAutocompleteSuggestion: function () {
    var $selected = $("#autocomplete-suggestions .autocomplete-item.selected");
    if ($selected.length === 0) {
      $selected = $("#autocomplete-suggestions .autocomplete-item").first();
    }
    this.insertAutocompleteSuggestion($selected.text());
  },

  insertAutocompleteSuggestion: function (suggestion) {
    var $code = $("#code");
    var cursorPosition = $code[0].selectionStart;
    var textBeforeCursor = $code.val().substring(0, cursorPosition);
    var textAfterCursor = $code.val().substring(cursorPosition);
    var lastLine = textBeforeCursor.split(";").pop();
    var lastWord = lastLine.split(/\s+/).pop();
    ...
    $code.val(newText);
    $code[0].setSelectionRange(
      cursorPosition - lastWord.length + replacement.length,
      cursorPosition - lastWord.length + replacement.length
    );
    this.hideAutocompleteSuggestions();
    $code.focus();
