var game = {
  ...
  setHandlers: function() {
    ...
    $('#editor').on('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', () => {
      $(this).removeClass();
    });
    ...
    $('.tooltip, .toggle, #level-indicator').on('click', (e) => {
      e.stopPropagation();
    });
  ...
  loadDocs: () => {
    $('#instructions code').each(() => {
      var code = $(this);
      var text = code.text();

      if (text in docs) {
        code.addClass('help');
        code.on('mouseenter', (e) => {
